...
export function openArtWorkLabel(matDialog: MatDialog, { picUrl, frameStyle, content, isRecTemplate, previewPic}: ArtworkLabel) {
    matDialog.open(ArtWorkLabelPopupComponent, {
      panelClass: 'art-work-label',
      autoFocus: false,
      data: {
        picUrl, frameStyle, content, isRecTemplate,
        minWidth: previewPic?.width || 'inherit',
        minHeight: previewPic?.height || 'inherit'
      }
    });
  }
